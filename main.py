...
def parseArgs():
    global _input_file, _output_dir, _format, _log_file, _target
    global _output_filename, _search_mode
    ...
    if args.help:
        ...
        print("Use: %s [hifot]" % basename,
            ...
            "\n\n  [-i, --input] <input_file> - CRIS input file (only xls)."
            "\n      default: ./data/dataCRIS.xls"

            "\n\n  [-o, --output-dir] <output_dir> - output directory. Will be "
            "created if doesn't exist.\n      default: ./results",

            "\n\n  [-n, --output-name, --filename] <output_name> - output name "
            "of files (without extension). \n      default: \"researchers\""
              
            "\n\n  [-f, --format] [csv,json,xls] - output format. Multiple "
            "options allowed (comma separated).\n    default: \"csv,json,xls\""

            "\n\n  [-l, --log] <filename> - enable logging and store it in the "
            "specified path."

            "\n\n  [-t, --target] [scholar,rg] - Scrap target. Multiple options"
            "allowed (comma separated)."
            "\n      scholar - Get personal data and stats from Google Scholar."
            "\n      rg - Get personal data and stats from Research Gate"
            "\n      default: all"
        )
        exit(0)
    ...
    if args.search: _search_mode = True
...
def configure_loggin():
    ...
    logging.basicConfig(handlers=logging_handlers, level=logging.DEBUG,
                        datefmt='%H:%M:%S', format='%(asctime)s - %(message)s')


def check_rg_researchers_url(researchers):
    not_valid = [r for r in researchers if r.rg_url and not
                                              is_valid_rg_profile_url(r.rg_url)]
...
def scrap (researchers, url_attribute_name, data_attribute_name, scraper):
    ...
    for index, r in enumerate(researchers):
        ...
        print("\rScraping with %s [%s/%s]: %s " %
              (scraper.__class__.__name__, index+1, size, r.crisid), end='')
        ...
        if not (data['personal_data'] or data['stats'] or data['articles']):
            wmessage = ("[No data for %s - %s %s. "
                       "Check if profile still exists] URL: %s" %
...
def store_results(researchers):
    if 'json' in _format:
        json_writer.store_researchers(
            researchers, _output_dir + "/" + _output_filename + ".json")
    if 'csv' in _format:
        csv_writer.store_researchers(
            researchers, _output_dir + "/" + _output_filename + ".csv")
    if 'xls' in _format:
        xls_writer.store_researchers(
            researchers, _output_dir + "/" + _output_filename + ".xls")
...
if __name__ == "__main__":
    ...
    if 'scholar' in _target:
        scrap([r for r in researchers if r.scholar_url
               and is_valid_scholar_profile_url(r.scholar_url)],
              "scholar_url", "scholar_data", ScholarScraper())

    # if 'rg' in _targets:
    #     scrap([r for r in researchers if r.rg_url and
    #     is_valid_rg_profile_url(r.rg_url)],
    ...
    print("Tiempo de carga: ",
          time.strftime("%H:%M:%S", time.gmtime(end_load - start_load)))
    print("Tiempo de scrap: ",
          time.strftime("%H:%M:%S", time.gmtime(end_scrap - start_scrap)))
    print("Tiempo de guardado: ",
          time.strftime("%H:%M:%S", time.gmtime(end_store - start_store)))