...
def get_stats(url):
    ...
    if about_section:
        stats = html.findAll('div', {'class': "nova-e-text nova-e-text--size-xl nova-e-text--family-sans-serif nova-e-text--spacing-none nova-e-text--color-inherit"})
        if stats:
            stats = {
...
def get_personal_data(url):
    ...
    personal_info = html.find('div', {'class': "nova-o-grid__column nova-o-grid__column--width-4/12@xl-only nova-o-grid__column--width-5/12@m-up nova-o-grid__column--width-12/12@s-up"})

    if (personal_info): 
        personal_info = personal_info.find('div', {'class': "nova-e-list__item nova-v-institution-item__info-section-list-item"})
        if personal_info:
            personal_data['position'] = personal_info.get_text()

    study_fields = html.findAll('a', {'class': "nova-e-badge nova-e-badge--color-grey nova-e-badge--display-block nova-e-badge--luminosity-medium nova-e-badge--size-l nova-e-badge--theme-ghost nova-e-badge--radius-full"})
